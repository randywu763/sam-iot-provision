...
#class MchpmchpZTKitDevice(hid.device):
class MchpmchpZTKitDevice():
    def __init__(self, device):
        super(MchpmchpZTKitDevice, self).__init__()
        self.device = device
        self.report_size = 64
    ...
    def open(self, vendor_id=DEVICE_HID_VID, product_id=DEVICE_HID_PID):
        """Opens HID device for the mchp Zero-touch Kit. Adjusts default VID/PID for the kit."""
        self.app_responses = {}
    ...
    def kit_read_app_no_error(self, id):
        ...
        if resp['error'] is not None:
            raise MchpmchpZTKitError(resp['error'])
        return resp
...
class MchpmchpZTKitError(Exception):
    def __init__(self, error_info):
        self.error_code = error_info['error_code']
        self.error_msg  = error_info['error_msg']
        super(MchpmchpZTKitError, self).__init__('Kit error %d: %s' % (self.error_code, self.error_msg))
