...
def aws_register_signer(
    ...
    if not os.path.isfile(signer_ca_key_path):
        raise AWSZTKitError('Failed to find signer CA key file, ' + signer_ca_key_path + '. Have you run ca_create_signer_csr first?')
    ...
    if not os.path.isfile(signer_ca_cert_path):
        raise AWSZTKitError('Failed to find signer CA certificate file, ' + signer_ca_cert_path + '. Have you run ca_create_signer first?')
