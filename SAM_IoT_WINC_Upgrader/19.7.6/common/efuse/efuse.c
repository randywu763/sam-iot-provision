...
static int load_efuse_to_regs(uint8 bankIdx)
{
    int retry;
    uint32 bankAddr;
    uint32 val32;
    ...
    for (bankIdx = 0; bankIdx < NUM_EFUSE_BANKS; bankIdx++) {
        ...
        if ((efuse_struct->bank_used == 1) && (efuse_struct->bank_invalid == 0))
        {
            ...
            if(efuse_struct->ver == 0) {
                ...
                efuse_struct->bIsIQAmpValid = (bank_data[2] >> 15) & BIT0;
			    if(efuse_struct->bIsIQAmpValid)
				    efuse_struct->IQAmpdB = (bank_data[2] >> 2) & 0x1fff;
			
			    efuse_struct->bIsIQPhaValid = (bank_data[2] >> 1) & BIT0;
			    if(efuse_struct->bIsIQPhaValid)
				    efuse_struct->IQPhaDeg = ((bank_data[3] >> 20) & 0xfff) | ((bank_data[2] & BIT0) << 12);
...
{
	if(g_efuse_struct.bIsIQAmpValid)
		nmi_bus_write(0x160CE4, g_efuse_struct.IQAmpdB);

	PRINTK("AIC CORR (%s)= %x", (g_efuse_struct.bIsIQAmpValid)?"Efuse":"FW", nmi_bus_read(0x160CE4));

	if(g_efuse_struct.bIsIQPhaValid) 
		nmi_bus_write(0x160CE0, g_efuse_struct.IQPhaDeg);

	PRINTKD(",PIC= %x\n", nmi_bus_read(0x160CE0));
}
