...
static tstrX509AlgID	gastrSecAlgorithms[] = 
{
	...
	// We only support MD5 for TLS if we allow TLS ver < 1.2.
	// We only allow TLS ver < 1.2 if we support RSA ciphersuites
...
Date
	20 March 2013
*********************************************************************/
TLS_CLIENT_API static tstrX509AlgID* Cert_DecodeAlgID
(
...
Date
	7 March 2013
*********************************************************************/
TLS_CLIENT_API static sint8 Cert_DecodeDistinguishedName
(
tstrAsn1Context				*pstrX509Asn1Cxt,
uint32						u32Size,
...
)
{
	...
	if((pstrX509Asn1Cxt != NULL) && (pstrDN != NULL))
	{
		...
		if(strElem.u8Tag == ASN1_OBJECT_IDENTIFIER)
		{
			...
			pstrDN->u8Type = 0;
			if(strElem.u32Length <= sizeof(au8CmnNameID))
			{
			...
			ASN1_GetNextElement(pstrX509Asn1Cxt, &strElem);
			/* Length of pstrDN->pu8Name buffer is given by u32Size. */
			if(strElem.u32Length <= u32Size)
			{
...
Date
	7 March 2013
*********************************************************************/
TLS_CLIENT_API static sint8 Cert_DecodeX520Name
(
tstrAsn1Context	*pstrX509Asn1Cxt,
uint32			u32Size,
...
Date
	22 March 2013
*********************************************************************/
TLS_CLIENT_API static sint8 Cert_DecodeRSAPubKey
(
tstrAsn1Context		*pstrX509Asn1Cxt,
		...
		if(strElem.u8Tag == ASN1_SEQUENCE)
		{
			...
			if(strElem.u8Tag == ASN1_INTEGER)
			{
				...
				if(pstrRsaPublicKey->pu8N != NULL)
				{
					...
					while((*pstrRsaPublicKey->pu8N == 0) && (strElem.u32Length > 0))
					{
					...
					if(strElem.u8Tag == ASN1_INTEGER)
					{
						...
						if(pstrRsaPublicKey->pu8E != NULL)
						{
							...
							while((*pstrRsaPublicKey->pu8E == 0) && (strElem.u32Length > 0))
							{
...
Date
	21 Feb 2016
*********************************************************************/
TLS_CLIENT_API static sint8 Cert_DecodeECCPubKey
(
tstrAsn1Context		*pstrX509Asn1Cxt, 
tstrAsn1Element		*pstrKeyParam,
tstrMemPool			*pstrMemPool,
...
)
{
	...
	if((pstrX509Asn1Cxt != NULL) && (pstrEcdsaKey != NULL) && (pstrKeyParam != NULL) && (pstrMemPool != NULL))
	{
		...
		if(strElem.u8Tag == ASN1_BIT_STRING)
		{
			...
			if(u8PointFmt == 0x04)
			{
				uint32 u32CoordLength;
				if((strElem.u32Length < 2) || (strElem.u32Length & 1))
				{
					goto _END__;
				}
				u32CoordLength = (strElem.u32Length - 2) >> 1;
				if(u32CoordLength > ECC_POINT_MAX_SIZE)
				{
					goto _END__;
				}
				pstrEcdsaKey->strQ.u16Size = u32CoordLength;
				ASN1_Read(pstrX509Asn1Cxt, u32CoordLength, pstrEcdsaKey->strQ.X);
				ASN1_Read(pstrX509Asn1Cxt, u32CoordLength, pstrEcdsaKey->strQ.Y);
				s8Ret = X509_SUCCESS;
			}
			else
			{
...
Date
	7 March 2013
*********************************************************************/
TLS_CLIENT_API static sint8 Cert_DecodeSubjectPubKey
(
tstrAsn1Context		*pstrX509Asn1Cxt,
uint32				u32Size,
tstrMemPool			*pstrMemPool,
...
Date
	23 March 2013
*********************************************************************/
TLS_CLIENT_API static sint8 Cert_DecodeValidity
(
tstrAsn1Context	*pstrX509Asn1Cxt, 
uint32			u32Size,
...
*/
	...
	if((pstrX509Asn1Cxt != 0) && (pstrCer != NULL))
	{
		...
		ASN1_GetNextElement(pstrX509Asn1Cxt, &strElem);
		if(strElem.u32Length < sizeof(au8Time))
		{
			/* Start time. */
			...
			if(strElem.u32Length < sizeof(au8Time))
			{
				...
				else if(X509_GET_SYS_TIME(&strSysTime) == M2M_SUCCESS)
				{
				...
				{
					pstrCer->u8ValidityStatus = X509_STATUS_TIME_UNKNOWN;
				}
				s8Ret = X509_SUCCESS;
			}
		}
...
Date
	7 March 2013
*********************************************************************/
TLS_CLIENT_API static sint8 Cert_DecodeTBSCertificate
(
tstrAsn1Context	*pstrX509Asn1Cxt,
uint16			u16CertSize,
	...
	if((pstrX509Asn1Cxt != NULL) && (pstrCert != NULL))
	{
		...
		*/
		while(u16ReadOffset < u16CertSize)
		{
			...
			if(enuCertState == CERT_VERSION_PENDING)
			{
				/*
					version
				*/
			...
			else if(enuCertState == CERT_SERIAL_PENDING)
			{
				/*
					serialNumber
				*/
			...
			else if(enuCertState == CERT_SIGNATURE_PENDING)
			{
				/*
					signature
				*/
			...
			else if(enuCertState == CERT_ISSUER_PENDING)
			{
				/*
					issuer
				*/
			...
			else if(enuCertState == CERT_VALIDITY_PENDING)
			{
				/*
					validity
				*/
			...
			else if(enuCertState == CERT_SUBJECT_PENDING)
			{
				/*
					subject
				*/
			...
			else if(enuCertState == CERT_SUBECTKEYINFO_PENDING)
			{
				/*
			...
			else if(enuCertState == CERT_EXTENSIONS_PENDING)
			{
...
)
{
	...
	if((pstrX509Asn1Cxt != NULL) && (pu8Sig != NULL))
	{
		...
		if((strElem.u8Tag == ASN1_SEQUENCE) && (strElem.u32Length <= u16TmpSz))
		{
			...
			{
				ASN1_GetNextElement(pstrX509Asn1Cxt, &strElem);
				/* We only support curves on 256-bit fields, so we lose nothing by rejecting long signatures here. */
				if((strElem.u8Tag == ASN1_INTEGER) && (strElem.u32Length < u16TmpSz) && (strElem.u32Length > 0) && (strElem.u32Length < 256))
				{
					/* Format 1 byte length, followed by integer (with leading 0 stripped). */
					uint8	*pu8IntLen = pu8Sig;
					uint8	u8IntLen = 0;

					u16TmpSz -= strElem.u32Length;
					pu8Sig++;
					u16SigSz++;
					...
					u8IntLen += (uint8)strElem.u32Length;
					*pu8IntLen = u8IntLen;
...
Date
	7 March 2013
*********************************************************************/
TLS_CLIENT_API static sint8 Cert_DecodeSignature
(
tstrAsn1Context	*pstrX509Asn1Cxt, 
uint16 			u16SigSize, 
tstrX509Cert 	*pstrCert
)
{
	...
	if((pstrX509Asn1Cxt != NULL) && (pstrCert != NULL))
	{
		...
		ASN1_GetNextElement(pstrX509Asn1Cxt, &strElem);
		if((strElem.u8Tag == ASN1_BIT_STRING) && (strElem.u32Length < u16SigSize) && (strElem.u32Length > 0))
		{
			/* Jump one byte. 
			*/
			ASN1_Read(pstrX509Asn1Cxt, 1, NULL);
...
Date
	12 April 2016
*********************************************************************/
static sint8 Cert_ComputeTBSCertHash(tstrTlsBuffer *pstrCrtBuff, tstrTLSBufferPos *pstrTBSCrt, uint16 u16TBSSize, tstrX509Cert *pstrCert)
{
	sint8	ret = X509_FAIL;
			...
			if(fpHash != NULL)
			{
				...
				if(pstrCert->pu8Hash != NULL)
				{
					...
					*/
					...
					while(u16TBSSize > 0)
					{
						u16ReadSize = TLS_BufferReadCurrEntry(pstrCrtBuff, u16TBSSize, &pu8TmpBuf);
...
TLS_CLIENT_API void X509Cert_Dump(tstrX509Cert *pstrCert)
{
	if(pstrCert != NULL)
	{
...
#endif	/* __CRYPTO_SHA224_SUPPORT__ */
		else if(pstrCert->enuHashAlg == HASH_ALG_SHA256)
		{
			TLS_LOG("SHA256");
		}
#ifdef __CRYPTO_SHA384_SUPPORT__
		else if(pstrCert->enuHashAlg == HASH_ALG_SHA384)
		{
			TLS_LOG("SHA384");
		}
#endif	/* __CRYPTO_SHA384_SUPPORT__ */
#ifdef __CRYPTO_SHA512_SUPPORT__
		else if(pstrCert->enuHashAlg == HASH_ALG_SHA512)
		{
			TLS_LOG("SHA512");
		}
#endif	/* __CRYPTO_SHA512_SUPPORT__ */
		TLS_LOG("\n");
		...
		{
			TLS_LOG("PUBLIC KEY\n\t");
			M2M_DUMP_BUF("ECDSA", pstrCert->strPubKey.strEccKey.strQ.XY, (pstrCert->strPubKey.strEccKey.strQ.u16Size * 2)); 
		}
