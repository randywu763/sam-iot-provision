...
#define BI_API			M2M_DATA_SEC_API
...
****************************************************************************************/
	...
	if((u32Y0 != 0) && ((u32X0 != 0) || (u32X1 != 0)))
	{
		...
		/* Perform devision of the Least significant digit. */
		for(i = 31; i >= 0; i --)
		{
				...
				*======*======*======*======*/
				for(i = n; i > t ; i--)
				{
					...
					*======*======*======*======*======*/
					...
					{	
						// TODO: This path is not tested at all. In all tetsing and actual scenarios it is not entered at all.
						/*=======================	*
						* STEP 3.4						*
						*	If X < 0 Then					*
						*		- Q[i-t-1] <== Q[i-t-1] - 1;	*
...
Version
	1.0
	2.0 30 May 2013 -> Use Different sizes for inputs to optimize looping.
...
Version
	1.0
	2.0 Eliminating stack usage by the function and use scratch memory instead.
...
)
{
	...
	/* BI Hw seems (empirically) to require N and E to begin with non-zero words (but note that N
	 * gets pre-padded with 0's if it is shorter than X).
	 * BI Sw does not have those requirements, but the execution time is unreasonably long if N is
	 * shorter than X.
	 * So in either case we should strip off leading zeroes, then compare lengths of X and N. */
	while((u32NSize>0) && (*pu8N==0))
	{
		u32NSize--;
		pu8N++;
	}
	...
	if(u32ESize==0)
		return BI_FAIL;
#ifdef __HW_BIGINT_ENGINE__
	/* Check size requirements for using Hw. */
	if((u32XSize <= 256) && (u32ESize <= 256) && (u32NSize <= 256) && (u32XSize < u32NSize+4))
	{
		s32Ret = BI_ModExpHW(pu8X, u32XSize, pu8E, u32ESize, pu8N, u32NSize, pu8Y, u32YSize);
	}
	else
#endif
	{
