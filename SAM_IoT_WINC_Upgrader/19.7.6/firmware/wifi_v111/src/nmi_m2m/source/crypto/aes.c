...
static const uint8 g_isbox[256] = {
...
static const uint8 g_logtable[256] = {
...
static const uint8 g_alogtable[256] = {
...
static const uint32 g_idxe[4][4] = {
  { 0, 1, 2, 3 },
  { 1, 2, 3, 0 },
  { 2, 3, 0, 1 },
  { 3, 0, 1, 2 }
...
static const uint32 g_idxd[4][4] = {
  { 0, 1, 2, 3 },
  { 3, 0, 1, 2 },
  { 2, 3, 0, 1 },
  { 1, 2, 3, 0 }
...
/*                                                                           */
/*****************************************************************************/
static void inv_mix_column(uint32 *a, uint32 *b)
{
  uint32 i = 0;
  uint32 j = 0;
  uint8 c[4][4];
  ...
  for(j = 0; j < 4; j++)
  {
      for(i = 0; i < 4; i++)
      {
          c[j][i] = mul(0xE, (uint8)((a[j] >> i * 8) & 0xFF))
                  ^ mul(0xB, (uint8)((a[j] >> ((i + 1) % 4) * 8) & 0xFF))
                  ^ mul(0xD, (uint8)((a[j] >> ((i + 2) % 4) * 8) & 0xFF))
                  ^ mul(0x9, (uint8)((a[j] >> ((i + 3) % 4) * 8) & 0xFF));
    }
  }
...
/*                                                                           */
/*****************************************************************************/
static void key_addition_8to32(uint8 *txt, uint32 *key, uint32 *out)
{
  uint32 i   = 0;
  uint32 j   = 0;
  uint32 val = 0;
...
/*                                                                           */
/*****************************************************************************/
static void key_addition32(uint32 *txt, uint32 *key, uint32 *out)
{
  uint32 i = 0;
...
/*                                                                           */
/*****************************************************************************/
static void key_addition32to8(uint32 *txt, uint32 *key,
                                         uint8 *out)
{
  uint32 i   = 0;
  uint32 j   = 0;
...
/*                                                                           */
/*****************************************************************************/
void aes_encrypt(aes_context_t *ctx, uint8 *plaintext, uint8 *ciphertext)
{
    uint32 r = 0;
    uint32 j = 0;
    uint32 e = 0;
    ...
    {
        e = wtxt[j] & 0xFF;
        e |= (wtxt[g_idxe[1][j]]) & (uint32)(0xFF << 8);
        e |= (wtxt[g_idxe[2][j]]) & (uint32)(0xFF << 16);
        e |= (wtxt[g_idxe[3][j]]) & (uint32)(0xFF << 24);
        t[j] = e;
    }
...
/*                                                                           */
/*****************************************************************************/
void aes_decrypt(aes_context_t *ctx, uint8 *ciphertext, uint8 *plaintext)
{
    uint32 r = 0;
    uint32 j = 0;
    uint32 e = 0;
    ...
    {
        e = wtxt[j] & 0xFF;
        e |= (wtxt[g_idxd[1][j]]) & (uint32)(0xFF << 8);
        e |= (wtxt[g_idxd[2][j]]) & (uint32)(0xFF << 16);
        e |= (wtxt[g_idxd[3][j]]) & (uint32)(0xFF << 24);
        t[j] = e;
    }
