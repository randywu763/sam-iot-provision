...
#define MD4_INTERMEDIATE(A,B,C,D,W,K,S)     \
{	\
	W 	+= A + K;	\
	W 	= SHA_ROTL(W, S);		\
	A 	= D;\
 ...
 */
static const uint32 K_SHA512[][2] = {
...
Return
	None
*********************************************************************/
void MD4ProcessBlock(uint32 *pu32Md4State, const uint8 *pu8MessageBlock)
{
	if((pu8MessageBlock != NULL) && (pu32Md4State != NULL))
	{
		...
		for(u8Idx = 0 ; u8Idx < 16 ; u8Idx ++)
		{
            ...
            MD4_INTERMEDIATE(A, B, C, D, W, 0, au8MD4Rot[(u8Idx % 4)]);
		}
				
		for(u8Idx = 0 ; u8Idx < 16 ; u8Idx ++)
		{
			W = MD4_G(B,C,D) + X[round2[u8Idx]];
            MD4_INTERMEDIATE(A, B, C, D, W, 0x5A827999, au8MD4Rot[(u8Idx % 4) + 4]);
		}
						
		for(u8Idx = 0 ; u8Idx < 16 ; u8Idx ++)
		{
			W = MD4_H(B,C,D) + X[round3[u8Idx]];
            MD4_INTERMEDIATE(A, B, C, D, W, 0x6ED9EBA1, au8MD4Rot[(u8Idx % 4) + 8]);
		}
...
void MD5ProcessBlock(uint32 *pu32Md5State,const uint8* pu8MessageBlock)
{
	...
	for(u8Idx = 0 ; u8Idx < 16 ; u8Idx ++)
	{
        ...
        MD5_INTERMEDIATE(A, B, C, D, W, T_MD5[u8Idx], au8MD5Rot[u8Idx % 4]);
	}

	for(u8Idx = 16 , u8Count = 1; u8Idx < 32 ; u8Idx ++ , u8Count += 5)
	{
		W = MD5_G(B,C,D) + X[u8Count & 0x0F];
        MD5_INTERMEDIATE(A, B, C, D, W, T_MD5[u8Idx], au8MD5Rot[(u8Idx % 4) + 4]);
	}
...
#pragma warning(suppress: 6385)
        MD5_INTERMEDIATE(A, B, C, D, W, T_MD5[u8Idx], au8MD5Rot[(u8Idx % 4) + 8]);
	}

	for(u8Idx = 48 , u8Count = 0; u8Idx < 64 ; u8Idx ++ , u8Count += 7)
	{
		W = MD5_I(B,C,D) + X[u8Count & 0x0F];
        MD5_INTERMEDIATE(A, B, C, D, W, T_MD5[u8Idx], au8MD5Rot[(u8Idx % 4) + 12]);
	}
...
void SHA1ProcessBlock(uint32* pu32Sha1State, const uint8* pu8MessageBlock)
{
	...
	*/
	...
	{
		W[t] += SHA1_CH(B, C, D);
        SHA1_INTERMEDIATE(A, B, C, D, E, W[t], K_SHA1[0])
	}

	for (t = 20; t < 40; t++) 
	{
		W[t] += SHA_PARITY(B, C, D);
        SHA1_INTERMEDIATE(A, B, C, D, E, W[t], K_SHA1[1])
	}

	for (t = 40; t < 60; t++) 
	{
		W[t] += SHA1_MAJ(B, C, D);
        SHA1_INTERMEDIATE(A, B, C, D, E, W[t], K_SHA1[2])
	}

	for (t = 60; t < 80; t++) 
	{
		W[t] += SHA_PARITY(B, C, D);
        SHA1_INTERMEDIATE(A, B, C, D, E, W[t], K_SHA1[3])
	}
...
)
{
    ...
    strIn.u16BufferSize = (uint16)u32DataLength;
